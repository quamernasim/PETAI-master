...
class ASPP(nn.Module):
    ...
    def __init__(self, C, depth, num_classes, conv=nn.Conv2d, norm=nn.BatchNorm2d, momentum=0.0003, mult=1/2):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, num_groups=None, weight_std=False, beta=False):
        ...
        self.conv_low_level = nn.Conv2d(512, 512, kernel_size=1, stride=1, bias=False)
        self.bn_low_level = self.norm(512)
        
        self.convUP2 = nn.Conv2d(1280, 512, kernel_size=1, stride=1, bias=False)
        self.bnUP2 = self.norm(512)
        self.convUP3 = nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding = 1, bias=False)
        self.bnUP3 = self.norm(512)
        self.convUP4 = nn.Conv2d(512, 6, kernel_size=1, stride=1, bias=False)
        self.bnUP4 = self.norm(6)
    ...
    def forward(self, x):
        ...
        low_level_feature = self.bn_low_level(low_level_feature)
        ...
        x = nn.Upsample((low_level_feature.shape[2], low_level_feature.shape[3]), mode='bilinear', 
                            align_corners=True)(x)
        x = torch.cat((x, low_level_feature), 1)
