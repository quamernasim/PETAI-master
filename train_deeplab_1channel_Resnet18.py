        ...
        if args.per_val != 0:
            ...
            if (epoch+1) % 5 == 0:
                ...
                torch.save({'epoch': epoch + 1,
                        'state_dict': model.state_dict(),
                        'optimizer': optimizer.state_dict(),}, model_fname % (epoch + 1))
    pd.DataFrame(pixel_acc).to_csv("val/metrics/pixel_acc.csv", index=False)
    pd.DataFrame(class_acc).to_csv("val/metrics/class_acc.csv", index=False)
    pd.DataFrame(mean_class_acc).to_csv("val/metrics/mean_class_acc.csv",index=False)
    pd.DataFrame(fre_weighted_iou).to_csv("val/metrics/freq_weighted_iou.csv",index=False)
    pd.DataFrame(mean_iou).to_csv("val/metrics/mean_iou.csv", index=False)
    pd.DataFrame(cls_iu).to_csv("val/metrics/cls_iu.csv", index=False)
    for i in range(len(confusion_mat)):
        name = "val/metrics/confusion_matrix/confusion_matrix_" + str(i) + ".csv"
