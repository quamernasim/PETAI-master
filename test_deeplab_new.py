...
def test(args):
    ...
    #    file_object.write('\n'.join(list_test))
    #    file_object.close()
    test_set = SectionLoader(is_transform=True, split=split,augmentations=None)
    ...
    with torch.no_grad():  # operations inside don't track history
        ...
        for i, (images, labels) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            if i in numbers:
                ...
                plt.savefig("test/original_image/{}.jpg".format(my_string))
                plt.close()
                ...
                plt.savefig("test/original_label/{}.jpg".format(my_string1))
                plt.close()
                ...
                plt.savefig("test/predicted/{}.jpg".format(my_string2))
                plt.close()
    ...
    #np.save("test/metrics/pred_class.npy", pred_class)
