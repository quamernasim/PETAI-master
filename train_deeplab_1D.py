...
def train(args):
    ...
    if args.train:
        ...
        criterion = core.loss.cross_entropy
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            loss = criterion(outputs, target=labels, weight=class_weights)
            #loss = loss_fn(input=outputs, target=labels, weight=class_weights)
            ## lossed edited by Tannistha
            loss_fn.update(loss.item(), args.batch_size)
            ...
            if (i) % 20 == 0:
                print('epoch: {0}\t\t'
                  'iter: {1}/{2}\t\t'
                  'Cross Entropy Loss: {3:.4f}\t\t'
                  'Average Meter Loss: val = {loss.val:.4f} (ema = {loss.ema:.4f}) (avg = {loss.avg:.4f})'.format(
                  epoch + 1, i + 1, len(trainloader), loss.item(), loss=loss_fn))
